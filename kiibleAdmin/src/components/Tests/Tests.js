...
function genSuccessHTMLOutput(response) {
	...
	return `${"<h5>Status:</h5> " +
              ...
              "<h5>Headers:</h5>" +
              `<pre>${JSON.stringify(response.headers)}</pre>` +
              "<h5>Data:</h5>" +
...
class Tests extends Component {
	...
	callPostContact(event) {
		...
		const contactOwnerId = this.state.contactOwnerId;
		...
		callingParams.append("email", email);
		callingParams.append("contactOwnerId", contactOwnerId);
	...
	render() {
		return (
			<div className='Tests'>
				...
				<Grid fluid="true" summary="Administration Tests page">
					<Row className='title-bar'>
						<Col md={12}>
							<Label>Press each buttton to call corresponding webservice </Label>
						</Col>
					</Row>
					<Row className='Wstest1'>
						<Col md={6}>
